...
q <- train_set %>% 
  group_by(movieId) %>% 
  summarise(movie_bias = round(mean(rating - mu), 1))
train_set <- train_set %>% 
...
q <- train_set %>% 
  group_by(userId) %>% 
  summarise(user_bias = round(mean(rating - mu), 1))
train_set <- train_set %>% 
...
fit <- train(rating ~ user_bias + movie_bias,
             data = train_set,
             method = "xgbTree",
             tuneGrid = xgbGrid,
...
final_xgb_predictions <- data.frame(userId, movieId, xgb_pred = q)
...
fit <- train(rating ~ userId + movieId,
             data = mjb_train_set,
             method = "nnet",
             trControl = fitControl, 
             preProcess = c("center", "scale"),
...
RMSE_nnet <- RMSE(test_set$rating, q)

ggplot(data = test_set, aes(x = test_set$rating, y = q)) +
       geom_point()
