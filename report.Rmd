...
#as(train_set[1:10,1:10], "matrix")

rec.model <- Recommender(data = r,
                         method = "IBCF",
                         parameter = list(method = "Cosine"))

#saveRDS(rec.model, file = "./rda/small_IBCF_model.rds")

#getModel(rec.model)$sim
#image(getModel(rec.model)$sim)
...
r2 <- train_set %>% 
  filter(userId %in% test_set$userId) %>% 
  as("realRatingMatrix")

pred <- predict(rec.model, r2, type = "ratings")

# convert from matrix to df to make joins easier
# final mutate is a KLUDGE to get the proper userId
ibcf_preds <- as(pred, "matrix") %>% 
  as.data.frame.matrix() %>% 
  rownames_to_column(var = "userId") %>% 
  gather(key = movieId, value = rating, -userId) %>% 
  mutate(userId = substring(userId, 2))
...
# TODO: FIX THIS !!!!  The join isn't correct
...
test_set2 <- test_set %>% 
  left_join(ibcf_preds)

confusionMatrix(test_set2$pred_rating, test_set2$rating)
