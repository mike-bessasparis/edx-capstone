...
#
NAcol <- which(colSums(is.na(edx_working)) > 0)
...
calc_rating <- function(mu, b_i, b_u, month) {
  ...
  #
  # rounding thresholds (.4 and .7) optimized by multiple trails
  #
  pred <- ifelse(pred - as.integer(pred) < .4, 
                 floor(pred), 
          ifelse(pred - as.integer(pred) > .7,
                 ceiling(pred),
          as.integer(pred) + .5 ) )
...
q <- confusionMatrix(final_custom_predictions$custom_pred, test_set$rating)$overall["Accuracy"]
