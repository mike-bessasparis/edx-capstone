...
t1 <- as_datetime(min(train_set$timestamp))
...
calc_rating <- function(mu, b_i, b_u, month) {
  ...
  # if rated before 2013 then round to whole star, otherwise to 1/2 star
  pred <- ifelse(month < ymd("2013-01-01"), .5, 5)
...
confusionMatrix(mjb_p$rating, mjb_v$rating)
