...
# !!! only for local pc !!!
#library(readr)
#edx <- read_csv("train-tiny.csv", col_types = cols(X1 = col_skip()))
...
d3 <- interval(d1, d2)
num_bins <- ceiling(d3 / dyears(1))
b <- d1 + c(0:num_bins) * years(1)
...
get_bin <- function(d) {
  ...
  for (j in seq(1, length(d))) {
    for (i in seq(1, nrow(t_bins))) {
      if (d[j] %within% interval(t_bins$start[i], t_bins$end[i])) { bin[j] <- i }
    }
  }
  return(bin)
}

get_bin(ymd("2003-06-01"))

#
# find avg for each movie, bin
#
train_set <- train_set %>% 
  mutate(time_bin = get_bin(month))

# calc known b_i_time = the avg rating for each movie during each time bin
...
get_b_i_time <- function(m, b) {
  ...
  for (j in seq(1, length(m))) {
    m2 <- movie_time_bias %>% 
            filter(movieId == m[j] & time_bin == b[j])
    if_else(nrow(m2) != 0, 
            bias[j] <- m2$b_i_time,
            bias[j] <- 0)
  }
...
get_b_i_time(1, 13)
get_b_i_time(c(1, 1), c(2, 3))


q <- movie_time_bias %>% 
  filter(movieId == 1 & time_bin == 13)
nrow(q)
...
calc_rating <- function(mu, b_i, b_i_time, b_u, month) {
  
  # basic prediction
  pred <- mu + b_i + b_i_time + b_u
...
final_custom_predictions <- test_set %>% 
  mutate(time_bin = get_bin(month)) %>% 
  mutate(b_i_time = get_b_i_time(movieId, time_bin))
  left_join(movie_bias, by="movieId") %>%
  left_join(user_bias, by="userId") %>%
  mutate(custom_pred = calc_rating(mu, b_i, b_i_time, b_u, month)) #%>% 
