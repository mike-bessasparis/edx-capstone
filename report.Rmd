...
# build time bins
d1 <- round_date(as_datetime(min(edx$timestamp)), unit = "day")
d2 <- round_date(as_datetime(max(edx$timestamp)), unit = "day")
d3 <- interval(d1, d2)
num_bins <- ceiling(d3 / dyears(1))
b <- d1 + c(0:num_buckets) * years(1)
t_bins <- data.frame(binId = seq(1, num_buckets), 
                  start = b[1:num_buckets], 
                  end = b[2:(num_buckets + 1)])

get_bin <- function(d) {
  bin <- NA
  for (j in seq(1, length(d))) {
    for (i in seq(1, 13)) {
      if (d[j] %within% interval(mjb$start[i], mjb$end[i])) { bin[j] <- i }
    }
  }
  return(bin)
}
...
train_set <- train_set %>% 
  left_join(m2, by = c("movieId", "time_bin"))
...
#  left_join(t, by="month") %>% 
  ...
  mutate(custom_pred = calc_rating(mu, b_i, b_u, month)) %>% 
