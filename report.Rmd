...
How do rating change over time?
```{r}
...
edx2 %>% 
  mutate(time_bin = round_date(as_datetime(timestamp), unit = "month")) %>% 
  group_by(time_bin) %>% 
  summarise(avg_rating = mean(rating)) %>% 
...
# Time bias model to get mu for each month
...
predictions <- validation %>% 
  ...
  mutate(pred = round((monthly_mu + b_i + b_u) / .5) * .5) %>%
...
mjb_p <- data.frame(predictions)
l <- c(".5", "1", "1.5", "2", "2.5", "3", "3.5", "4", "4.5", "5")
mjb_p <- mjb_p %>% 
  mutate(predictions = factor(predictions, levels = l))

mjb_v <- validation %>% 
  mutate(rating = factor(rating, levels = l))

confusionMatrix(mjb_p$predictions, mjb_v$rating)
