...
#
userId <- as.character(test_set$userId)
pred <- sapply(userId, function(userId) {
                          return(predict(rec.model, r[userId], type = "ratings")) } )

# TODO: get the predicitons out of the sapply list
# convert from matrix to df to make joins easier
ibcf_preds <- as(pred[[1]], "matrix") %>% 
  as.data.frame() %>% 
