...
How does the number of movies reviewed on a particular date by a user (frequency) effect ?
```{r}
...
mjb <- edx_working %>% 
  ...
  summarise(F_i = n(), b_i_f = log(n()))

edx_working <- edx_working %>% 
  left_join(mjb)

edx_working %>% 
  ggplot(aes(x = F_i, y = rating)) +
  geom_boxplot(aes(group = F_i)) +
  theme_economist_white()
