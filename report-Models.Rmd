...
train_set <- feature_eng %>% 
   mutate(userId = as.factor(userId),
          movieId = as.factor(movieId)) %>% 
  select(-rating_date, -time_bin, -t_u)
...
# 
# train_set <- train_set %>% 
#   mutate(days_since_first_rating = timestamp - first_rating)
# 
# q <- test_set %>% 
#   group_by(userId) %>% 
#   summarise(first_rating = min(timestamp))
...
final_xgb_predictions <- data.frame(userId = as.character(test_set$userId), 
                                    movieId = as.character(test_set$movieId), 
                                    xgb_pred = q)

RMSE_xgb <- RMSE(test_set$rating, final_xgb_predictions$xgb_pred)

#
# add predictions to ensemble
#
all_model_predictions <- all_model_predictions %>% 
    left_join(final_xgb_predictions, by = c("userId", "movieId"))
...
################################
#
# calculate the user's mean rating date for use in
# dev_u(t) = sign(t - t_u) * |t - t_u| ^ beta, drift of a user's ratings over time
#

q <- train_set %>% 
  group_by(userId) %>% 
  summarize(t_u = as_date(mean(timestamp) / 86400)) # 86400 = seconds in a day (fast way to truncate timestamp)
...
#
# lookup function to find a b_i_time
#
...
#
calc_rating <- function(movieId, time_bin, mu, b_i, b_u, dev_u_t, rating_date, alpha) {
  
  # base prediction
  pred <- mu + b_i + b_i_time_matrix[movieId, time_bin] + b_u + alpha * dev_u_t

  # accuracy is better without this
  # but its somehow enabling the matrix lookup in this function call to work
  # so I changed the date to make it moot
  # if rated before 2003-05-15 then round to whole star
  pred <- ifelse(rating_date < ymd("1900-05-15"), 
                round(pred),
                pred)
...
#
beta <- 0.23 # parameter for dev_u_t
alpha <- -0.0046
...
final_custom_predictions <- test_set %>% 
  ...
  left_join(t_u, by = "userId") %>% 
  mutate(dev_u_t = as.integer(sign(difftime(rating_date, t_u))) * 
                               abs(as.numeric(difftime(rating_date, t_u, units = "days"))) ^ beta) %>% 
  mutate(custom_pred = calc_rating(movieId, time_bin, mu, b_i, b_u, dev_u_t, rating_date, alpha)) %>% 
  mutate(res = rating - custom_pred)

RMSE_custom <- RMSE(validation$rating, final_custom_predictions$custom_pred)



feature_eng <- train_set %>% 
  mutate(time_bin = get_bin(timestamp)) %>% 
  left_join(movie_bias, by="movieId") %>%
  left_join(user_bias, by="userId") %>% 
  left_join(t_u, by = "userId") %>% 
...
saveRDS(all_model_predictions, file = "./rda/mjb.rds")

all_model_predictions <- all_model_predictions %>% 
  select(userId, movieId, rating.x, custom_pred, xgb_pred)

mjb <- all_model_predictions %>% 
  select(-userId, -movieId, -rating.x) %>% 
  as.matrix()
