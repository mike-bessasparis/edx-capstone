...
fit <- randomForest(rating ~ movieId, data = train_set) 
plot(fit)

q <- predict(fit, test_set)

final_rf_predictions <- data.frame(userId = test_set$userId, movieId = test_set$movieId, rf_pred = q)
